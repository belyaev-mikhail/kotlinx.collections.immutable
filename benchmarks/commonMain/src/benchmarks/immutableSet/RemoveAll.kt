...
open class RemoveAll {
    ...
    @Benchmark
    fun removeAllDisjoint(): ImmutableSet<IntWrapper> {
        return set - setDisjoint
    }

    @Benchmark
    fun removeAllMixed(): ImmutableSet<IntWrapper> {
        return set - setMixed
    }

    @Benchmark
    fun removeAllCopy(): ImmutableSet<IntWrapper> {
        return set - setCopy
    }

    @Benchmark
    fun removeAllDisjointOld(): ImmutableSet<IntWrapper> {
        return set.mutate { for(e in setDisjoint) it.remove(e) }
    }

    @Benchmark
    fun removeAllMixedOld(): ImmutableSet<IntWrapper> {
        return set.mutate { for(e in setMixed) it.remove(e) }
    }

    @Benchmark
    fun removeAllCopyOld(): ImmutableSet<IntWrapper> {
        return set.mutate { for(e in setCopy) it.remove(e) }
    }
}